...
// A class for representing affine functions on a right Dim-dimensinonal simplex
template< unsigned int Dim >
struct SimplexFunction
{
	// Set the function's coefficients from its values
	SimplexFunction( void );
	...
	// Evaluate the function at a prescribed point
	...
	// Returns the dual of the function
	Point< double , Dim+1 > dual( void ) const;
...
template< unsigned int Dim >
SimplexFunction< Dim >::SimplexFunction( void ) : _c(0) {}
...
template< unsigned int Dim >
Point< double , Dim+1 > SimplexFunction< Dim >::dual( void ) const
{
	Point< double , Dim+1 > dual;
	dual[0] = -_c;
	for( unsigned int d=0 ; d<Dim ; d++ ) dual[d+1] = _v[d];
	return dual;
}
