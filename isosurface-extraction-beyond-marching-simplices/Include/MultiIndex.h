...
struct MultiIndices
{
	...
	MultiIndices( unsigned int N ) : _N(N) { if( N<K ) ERROR_OUT( "Number of indices cannot be less than size: " , K , " <= " , N ); }

	unsigned int size( void ) const{ return MultiIndex< K >::Choose( _N , K ); }
